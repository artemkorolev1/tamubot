...
### 2026-06-15 · Sub-floor section backward-merges across sections, hiding course content · BP_OVERREACH
  impact: iter_12 #1 class (17 blockers). A tiny course-specific section (e.g. "Course Specific Late Work Policy" / a one-line "Course Schedule") backward-merged into the adjacent generic policy chunk and inherited its header ("Late Work Policy"); header-anchored boilerplate tagging then hid the WHOLE chunk, dropping course content from RAG. NB handoff mis-located this in `_merge_small` (dead code for v6b — `silver_chunk_semantic` calls `chunk_semantic`); tagging-side guard can't help — the course text carries no %/date/URL for `find_course_specific_signal`.
  fix:    APPLIED — `chunk_semantic` (`chunker_v4.py`) now backward-merges a sub-`min_chunk_tokens` fragment ONLY when its `header_path` matches the previous chunk's; a cross-section fragment becomes its own chunk (never dropped, never cross-merged) so its own header guards it from header-anchored over-hiding (verified: "course specific late work policy"/"course schedule" are NOT in the ≥4-dept header allow-list). Factored `_append_chunk`/`_merge_into_prev`; 3 regression tests (16 chunker tests green).
  check:  `v6b_silver_tag_no_boilerplate_overhide` (existing WARN) catches only %/date/weekday signals; this defect is structural (chunking) → caught by the new chunker unit tests.
  status: ✅ CONFIRMED end-to-end — container re-chunk+re-tag of all 100 (100/100 tagged, 0 fail) + visual sub-agent re-judge of the 17. Boilerplate 0/17→**12/17** pass (sibling-section merges fully resolved); dedup 17/17, chunking 16/17, fidelity 17/17. Corpus visible chunks 765→941 (+23%), 0/100 content lost, +16.1% chunks (mostly now-addressable metadata). The 4 cross-syllabus dedup collapses of the split-out sections were judged CORRECT generic-text dedup (not DUP_FALSE). The 5 residual boilerplate fails are a DIFFERENT class — see next entry. `golden_recall@5` still pending (needs live-Atlas re-ingest).

### 2026-06-15 · Course content as H3 child under a boilerplate H2 header is hidden · BP_OVERREACH
  impact: 5/17 re-judged stems still fail boilerplate AFTER the sibling-merge fix — course-specific content nested as an H3 CHILD of a generic boilerplate H2 is correctly kept with its parent by the chunker, then the parent's header anchors boilerplate tagging and hides the child. Pre-existing (failing in iter_12 too), NOT a regression. Splits into TWO sub-classes:
  cause-1 (4 stems · Docling reading-order float · owner `converters/docling_block_adapter.py`): ECEN_666 / CSCE_627 / ECEN_647 / CSCE_631 — Simple-Syllabus textbook CARDS (book-cover image + `### This material Is: Required` + ISBN/Authors/Publisher/URL, laid out two-column) are read by Docling AFTER the same-page plain-text headers, so `## Textbook and/or Resource Materials` is emitted EMPTY (immediately followed by `## Grading Policy`) and the card text floats to under `## Late Work Policy`. Confirmed identical signature in all 4 bronze md + PDF p3 (ECEN_666). FAITHFUL-but-misordered extraction.
  cause-2 (1 stem · genuine nesting · owner `util/tagging.py`): ISEN_625 — `### Simio Software` (+ simio.com URL) is GENUINELY the last item under `## Technology Support` in the source (PDF p8). Bronze is faithful; the whole Technology Support chunk is boilerplate-hidden, taking the URL-bearing course child with it.
  fix:    open — cause-1: bronze re-parenting heuristic — move an orphaned `### This material Is:` textbook-card run to immediately after the nearest preceding empty `## Textbook…` header (fits the existing recovery heuristics in docling_block_adapter). cause-2: tagging guard — don't suppress a boilerplate-headed chunk whose body holds a course-specific signal incl. URL/ISBN (extend `find_course_specific_signal` beyond %/date/weekday).
  check:  `v6b_silver_tag_no_boilerplate_overhide` (WARN) does NOT catch these — signals are %/date/weekday only (no URL/ISBN).
  status: 🔲 open · root-caused 2026-06-15 by the post-fix re-judge (CSCE_627, CSCE_631, ECEN_647, ECEN_666 = reading-order float; ISEN_625 = genuine nesting)
