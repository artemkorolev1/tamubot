...
### 2026-06-15 · Course content as H3 child under a boilerplate H2 header is hidden · BP_OVERREACH
  impact: 5/17 re-judged stems still fail boilerplate AFTER the sibling-merge fix — course-specific content nested as an H3 CHILD of a generic boilerplate H2 is correctly kept with its parent by the chunker, then the parent's header anchors boilerplate tagging and hides the child. Pre-existing (failing in iter_12 too), NOT a regression. e.g. ECEN_666 / CSCE_627 / ECEN_647 / CSCE_631 (textbook `### This material Is: Required` + ISBN under `## Late Work Policy`; the real `## Textbook and/or Resource Materials` header is empty), ISEN_625 (`### Simio Software` + simio.com URL under `## Technology Support`).
  fix:    open — two candidate owners: (1) bronze/extraction re-parents mis-placed blocks under the correct header (`assets/bronze_blocks.py` / `converters/docling_block_adapter.py`) — root fix for the textbook cases where the Textbook H2 is empty; (2) tagging guard: don't suppress a boilerplate-headed chunk that contains an H3 child carrying a course-specific signal incl. URL/ISBN (`util/tagging.py` + extend `find_course_specific_signal` to URL/ISBN). The chunker cannot fix it without violating section-unity (child genuinely belongs to parent).
  check:  `v6b_silver_tag_no_boilerplate_overhide` (WARN) does NOT catch these — its signals are %/date/weekday only (no URL/ISBN).
  status: 🔲 open · isolated 2026-06-15 by the post-fix re-judge (CSCE_627, CSCE_631, ECEN_647, ECEN_666, ISEN_625)
