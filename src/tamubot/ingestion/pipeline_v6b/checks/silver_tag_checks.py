...
@asset_check(asset="v6b_silver_tag_semantic", blocking=False, partitions_def=stem_partitions)
def v6b_silver_tag_no_duplicate_overhide(
    context: AssetCheckExecutionContext,
) -> AssetCheckResult:
    """Dedup over-hide gate (CSCE_629 class): a chunk flagged ``is_duplicate=True`` whose
    body carries an actionable section-specific signal (a URL, grade percentage, or
    concrete due date) — content that should stay visible in this section's retrieval
    rather than collapse to a cross-corpus canonical in another stem. The dedup guard in
    util/tagging prevents these; this is the regression guard. WARN for now; promote to
    blocking once the pass rate is confirmed across the golden set."""
    stem = context.partition_key
    dst = json.loads(paths.silver_tag_path(stem, "semantic").read_text(encoding="utf-8"))
    outcome = check_no_duplicate_overhide(dst["chunks"])
    n = outcome.metadata["overhide_count"]
    offenders = outcome.metadata["offenders"]
    detail = f"; e.g. {offenders[0]['header_path']} ({offenders[0]['signal']})" if n and offenders else ""
    return AssetCheckResult(
        ...
        severity=AssetCheckSeverity.WARN,
        description=(
            "no actionable signals in duplicate-flagged chunks"
            if outcome.passed
            else f"{n} duplicate chunk(s) carry actionable section-specific content{detail}"
        ),
        metadata=outcome.metadata,
    )
